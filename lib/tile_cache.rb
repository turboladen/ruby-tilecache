...
module TileCache
  ...
  class << self
    ...
    def initialize_cache
      ...
      fail InvalidConfiguration, 'Missing cache section in your configuration file' unless config
    ...
    def initialize_layers_pool
      ...
      fail InvalidConfiguration, 'Please configure at least one layer' unless layers
      ...
      unless valid_attributes.empty?
        config.except(:name).each_key do |a|
          unless valid_attributes.include?(a.to_s)
            fail InvalidConfiguration, "Unrecognized attribute in #{config[:name]}: #{a}"
          end
        end
      end
      ...
      unless required_attributes.empty?
        required_attributes.each do |a|
          unless config.include?(a.to_sym)
            fail InvalidConfiguration, "Missing attribute in #{config[:name]}: #{a}"
          end
        end
      end
    end
