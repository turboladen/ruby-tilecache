...
module TileCache
  class Layer
    ...
    def initialize(name, config)
      ...
      @size = config[:size].is_a?(String) ? config[:size].split(',').map { |s| Integer(s.strip) } : config[:size].map(&:to_i)
      @units = config[:units]
      @srs = config[:srs]
    ...
    def render_bbox(bbox)
      ...
      when Bounds
        ...
        fail ArgumentError, "Invalid argument for bbox: #{bbox.inspect}"
      end
    ...
    def level_for_bbox(bbox)
      ...
      if match = @resolutions.detect { |res| (res - bbox.resolution(*@size)).abs < max_diff }
        ...
        fail TileCache::InvalidResolution, "Can't find resolution index for #{bbox.resolution}. Available resolutions are #{@resolutions.join(', ')}."
      end
    end
    ...
    def parse_resolutions(res_list, _min_res, max_res, levels)
      case res_list
      when String
      ...
      when NilClass
        ...
        fail TileCache::Layers::InvalidConfiguration, "Invalid format of resolutions for layer #{@name}"
      end
    end
  end
end
